...
import { ArrowUp, Heart, Loader2, MessageCircle, Telescope } from "lucide-react";
...
function MessageBubble({
  ...
  return (
    ...
    >
      {!isUser ? <AiGuruAvatar size="md" className="border-rose/30 mt-0.5 shrink-0" /> : null}
      <div
        className={cn(
...
function TypingIndicator() {
  return (
    <div className="animate-in fade-in-0 flex items-start gap-2.5 duration-200 sm:gap-3">
      <AiGuruAvatar size="md" busy className="border-rose/30 shrink-0" />
      <div className="border-border/50 bg-card/95 dark:bg-card/80 shadow-soft rounded-[1.25rem] rounded-bl-md border px-4 py-3">
        <div className="flex items-center gap-1.5">
          <span className="bg-rose/70 h-1.5 w-1.5 animate-bounce rounded-full [animation-delay:0ms]" />
          <span className="bg-rose/70 h-1.5 w-1.5 animate-bounce rounded-full [animation-delay:150ms]" />
          <span className="bg-rose/70 h-1.5 w-1.5 animate-bounce rounded-full [animation-delay:300ms]" />
          <span className="text-muted-foreground ml-2 text-xs">
            {MARRIAGE_GURU_NAME} is thinking…
          </span>
        </div>
      </div>
    </div>
...
function ChatComposer({
  ...
  return (
    <div className="border-border/40 from-background via-background/95 to-background/80 shrink-0 border-t bg-gradient-to-t px-3 pt-3 pb-3 sm:px-4 sm:pb-4">
        ...
        <div className="scrollbar-hidden mb-3 flex gap-2 overflow-x-auto pb-0.5">
          {prompts.slice(0, 5).map((prompt) => (
            <button
              ...
              className="border-border/60 bg-card/70 hover:border-rose/45 hover:bg-rose/8 text-foreground/85 dark:bg-card/50 shrink-0 rounded-full border px-3.5 py-2 text-left text-[11px] leading-snug transition disabled:opacity-50 sm:text-xs"
            >
              {prompt}
            </button>
          ))}
      ...
      <form
        onSubmit={onSubmit}
        className="border-border/50 bg-card focus-within:border-rose/45 focus-within:ring-rose/20 dark:bg-card/60 shadow-soft flex items-end gap-2 rounded-[1.5rem] border p-1.5 transition focus-within:ring-2"
      >
        <textarea
          ref={textareaRef}
          value={draft}
...
export default function MarriageGuruPage() {
  const { setMode } = useWorkspaceMode();
  const [bundle, setBundle] = useState<MarriageBundle | null>(null);
  ...
  useEffect(() => {
    void fetch("/api/ai/marriage-guru")
      .then((r) => r.json())
      .then((json) => {
        if (!json.success) {
          setError(json.error?.message || `Failed to load ${MARRIAGE_GURU_NAME}`);
          return;
        }
        const data = json.data as MarriageBundle;
        setBundle(data);
        setMessages([
          {
            role: "assistant",
      ...
      .catch(() => setError(`Failed to load ${MARRIAGE_GURU_NAME}`))
  ...
  const chatPanel = (
    <section className="border-border/50 bg-card/40 dark:bg-card/30 shadow-soft dashboard-fill-h flex flex-col overflow-hidden rounded-[1.25rem] border backdrop-blur-xl sm:rounded-[1.75rem] lg:max-h-[min(760px,calc(100dvh-var(--dashboard-chrome)))]">
      <header className="border-border/40 from-card via-card to-rose/10 dark:to-rose/10 relative shrink-0 border-b bg-gradient-to-r px-4 py-3.5 sm:px-5">
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,color-mix(in_srgb,var(--rose)_14%,transparent),transparent_55%)]" />
        <div className="relative">
          <AiGuruHeader
            online
            subtitle={
              bundle?.hasChart && bundle?.chartSummary
      ...
      <div className="relative min-h-0 flex-1">
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_bottom_left,color-mix(in_srgb,var(--rose)_6%,transparent),transparent_45%)]" />
        <div
          ref={scrollRef}
          onScroll={onScroll}
          ...
          <div className="space-y-4 px-3 py-4 sm:space-y-5 sm:px-5 sm:py-5">
            {loading ? (
              <div
                ...
                role="status"
                aria-label={`Preparing ${MARRIAGE_GURU_NAME}`}
              >
            ...
            {!userAsked && !loading && messages.length <= 1 ? (
              <div className="border-border/40 from-rose/5 mx-auto max-w-sm rounded-2xl border border-dashed bg-gradient-to-b to-transparent px-4 py-4 text-center">
                <p className="text-muted-foreground text-xs leading-relaxed">
                  Ask one clear question — {MARRIAGE_GURU_NAME} answers briefly in simple words.
                </p>
              </div>
            ) : null}
  ...
  return (
    <div className="relative space-y-4 sm:space-y-5">
      <PageHeader
        ...
        title={MARRIAGE_GURU_NAME}
        description="Marriage timing, spouse themes, and alliance readiness — focused only on vivaha, not general astrology."
        actions={
          <div className="flex flex-wrap gap-2">
            ...
            <Button asChild variant="outline" size="sm">
              <Link href={routes.compatibility}>
      ...
      <div className="lg:hidden">
        <Tabs value={mobileTab} onValueChange={setMobileTab}>
          ...
          <TabsContent value="chart" className="mt-0">
            <GuruChartPanels data={bundle as ChartPanelsData | null} />
            {bundle?.disclaimer ? (
